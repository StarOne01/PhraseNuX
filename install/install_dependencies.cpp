...
string name;
cout << "Please enter the command to start the Program i.e, the word which you enter now will be used to call the program\n";
cin >> name;
std::ofstream file;
file.open(name);
...
system("bash start");
name = "chmod +x /usr/bin/" + name;
const char* nnamee = name.c_str();
system(nnamee);
}
else {
